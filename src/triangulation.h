...
public:
    ...
    // Punteros para los datos de salida (gestionados por main)
    int *seed_edges = nullptr;
    int *comp_sec_output_seed = nullptr;
               ...
               // he.target = target;
                ...
                if(neigh != -1){
                    for (std::size_t k = 0; k < 3; k++){
                        if(faces.at(3*neigh + k) == target && faces.at(3*neigh + (k + 1)%3) == origin){
                            he.twin = 3*neigh + k;
                            break;
                        }
                    }
...
public:
    ...
    Triangulation(const Triangulation &t) {
        ...
        // NOTA: No copiamos los punteros seed_edges, ya que son
...
void write_triangulation_to_off(Triangulation &output, std::string filename){
    ...
    for(int i = 0; i < output.n_halfedges; i++){
        if (!visited[i] && !output.is_border_face(i)) {
            ...
            int curr = i;
            do {
                if(curr >=0 && (unsigned)curr < visited.size())
                    visited[curr] = true;
                curr = output.next(curr);
            } while (curr != i);
        }
    }
    ...
    for(int i = 0; i < output.n_halfedges; i++){
        ...
        if (!visited[i] && !output.is_border_face(i)) {
            ...
            do {
                ...
                vertices_of_face.push_back(output.origin(current_he));
                current_he = output.next(current_he);
            } while (current_he != i);

            outFile << vertices_of_face.size();
            for(int v_index : vertices_of_face){
                outFile << " " << v_index;
            }
            outFile << std::endl;
        }
...
void write_compaction_file(Triangulation &output, std::string filename, int n_seed_edges){
    ...
    // Asumiendo que comp_sec_output_seed está lleno y tiene al menos n_seed_edges
    if (output.comp_sec_output_seed != nullptr) {
        for (int i = 0; i < n_seed_edges; i++) {
            outFile << output.comp_sec_output_seed[i] << std::endl;
        }
    }
    outFile.close();
